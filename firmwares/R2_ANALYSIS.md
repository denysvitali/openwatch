...
- **Channel-A `0x7d` is a deferred no-op, not UV history.** In the v14 deferred worker, `cmp r1,0x7d` at body `0x6f5e` branches directly to the common queue-advance path at `0x6f7c`; no handler call or long-response fragmenter is reached.
...
Channel-B sleep records use `0x27` as the only host request opcode. At v14
`0x96da`, the handler clamps `payload[0]` to `6` as the maximum day offset,
computes the base day as `today - maxOffset`, stores `payload[1]`, and checks
it at `0x96fc`. The day loops run from `baseDay` through today and write
`maxOffset - loopIndex` as each record's day delta. Only `payload[1] == 1`
enters the nap loop and emits a `0x3e` response at `0x977a..0x977c`; the
handler then always emits the `0x27` night response at `0x97fe..0x9800`. The
async worker has no direct `0x3e` request branch, so direct host `0x3e` frames
use the default NAK-code-0 path.
...
1. **`image_digest` algorithm** — the 32-byte `0x1c4` field is SHA-256-sized, but no SHA-256 init constants exist in the body (§8 crypto negatives). A 2026-07-07 radare2 pass found **no code or data references** to the digest region (`0x008261c4`) and no embedded digest-value bytes anywhere in the body. The runtime OTA body checks only the first container word, strips file offset `0x50`, stages the digest-containing region as raw data, and checks final staged length (`expected_size - 0x50`). The digest is therefore either (a) a build-host-only stamp that is never verified on-device, or (b) verified by ROM/bootloader code outside this OTA slice. A full lower-flash/bootloader dump is required to distinguish these. See `firmwares/_re/digest-and-boundary/evidence.md`.
...
3. ~~**Header constants**~~ — **resolved for the app body:** `const_5c = 0x7e6b4cf9` is the head of the constant GUID. `const_b4 = 0x1201a39e` appears only at header offset `0xb4`; radare2 found no body literal or xref, so it is header-only metadata. `const_228 = 0x0e85d101` is the first word of the boundary marker that sits exactly `0x50` bytes after `flash_app_end`; the marker precedes the allocator/runtime tail (`malloc`/`calloc`/`realloc` strings) and is not referenced by the app body. See `firmwares/_re/digest-and-boundary/evidence.md`.
...
6. ~~**BP history full-slot host path**~~ — **resolved negative for v14:** FEE7 `0x0d` low-switch case decodes to body `0x63fa`, which calls `0xde52` then the same compact `0x5ca4` sender used by Channel-A `0x0e`. `axt 0xde96` shows only `0x5ca4` calls the BP builder, and the BP descriptor pointer literal `0x00845ae4` appears only at `0xe0b0`. Static RE found no host-visible path exposing the remaining three bytes of each 4-byte BP slot; live cuff/manual correlation is still needed to assign semantics to the compact byte.
7. **Boot/vector region** — the real Cortex-M vector table, reset handler, and the `BootOnce` dual-bank logic live in flash ≤`0x826400`, outside both OTA bodies. Obtaining a full-flash dump would let us verify the load/verify path end-to-end.
8. ~~**`flash_app_end @0x22c` bootloader use**~~ — **resolved for the app body:** the pointer target is v13 body `0x21460` / v14 body `0x1f814`, the unique `0x0e85d101,0x00000001` marker immediately before allocator/runtime string and pointer tables (`malloc`, `calloc`, `realloc`, etc.). The marker sits exactly `0x50` bytes after the `flash_app_end` value in both builds. The runtime body has no direct xrefs to the pointer value or marker, so the app body does not enforce the boundary; it is bootloader/runtime metadata. See `firmwares/_re/digest-and-boundary/evidence.md`.
