...
6. ~~**BP history full-slot host path / 4-byte slot encoding**~~ — **resolved for v14:** wire path remains compact-only (FEE7 `0x0d` → `0x63fa` → `0xde52` → `0x5ca4` → `0xde96`; descriptor lit only at `0xe0b0`). Sole slot writer body `0xdd2c` persists `[compact,0,0,0]` from auto-measure tick `0xdd80` (`heart_rate_current_bpm` or PRNG `(r%5)+0x46`). Live synthetic sys/dia at `FUN_00834092` is notify-only. See `firmwares/_re/bp-slot-encoding/evidence.md`.
