...
| TouchControlReq / UVSettingReq | `0x3b` | sub+guard | →watch | read:`[01,00]`; write:`[02,00,value]`; nonzero second byte = batch/no-commit echo | echo/status: `pl[0]`=sub, `pl[1]`=guard, `pl[2]`=read/written value | Touch/UV config byte at `DAT_0082cfe8 + 8`. `0x3e` is not the Channel-A UV setting on H59MA v14. |
...
| SelfMarkerEcho | `0x90` | both | bare opcode | self-marker ACK `[0x90,0,...,0x90]` | Low-cost echo with byte 15 used as a marker; verified at v14 body offset `0x643e` and callee `0x16d2`. |
| ChecksumEcho | `0x91` | both | bare opcode | additive-checksum ACK `[0x91,0,...,cksum]` | Normal checksum echo pair for `0x90`; verified at v14 body offset `0x6446` and callee `0x16ee`. |
| NoResponsePlaceholder | `0x92` | →watch | bare opcode | none | Explicit no-op handler wired into the dispatcher; verified at v14 body offset `0x644e` and callee `0x1714` (`bx lr`). |
...
| RuntimeNoop | `0xc4` | →watch | bare opcode | none | Explicit no-op: dispatcher body offset `0x6504` calls `0xa062`, which is `bx lr`. |
| RuntimeFlagWrite | `0xc5` / `0xc8` / `0xc9` | →watch | `req[1]` | none | Writes `DAT_0082caec` runtime bytes: `0xc5` stores `req[1] == 1` at `+3`, `0xc8` stores `req[1] == 1` at `+4`, and `0xc9` stores raw `req[1]` at `+5`; verified at v14 body offsets `0x6518`, `0x650a`, and `0x6526`. |
| VendorMemReadSmall | `0xcd` | both | `[sub, len, addr u32BE]`; `sub==1`, len clamps to `14` | single `[0xcd, copied bytes..., cksum]` frame | Small arbitrary-address read, verified at v14 body offset `0x6534` and callee `0x5a12`. OpenWatch must not call this outside explicit developer/debug tooling. |
| FactoryTest | `0xce` | both | `[sub, param0, param1, len, data...]` | sub-dependent factory/test response | OEM factory/test entry point for subcommands `0x01`, `0x02`, `0x20`, `0x21`, and `0x22`; verified at v14 body offset `0x653c` and callee `0x58de`. |
...
| Compact NAK/status | cmd | watch→ | n/a | `[BC,cmd,01,00,crc16(error_code),error_code]` | One-byte Channel-B error/status frame from `FUN_0082ee00`. Unsupported commands use `error_code` as the NAK code; OTA response types use the same one-byte payload shape as normal status frames, so request context decides the final error message. |
...
> - Compact Channel-B NAK/status (`FUN_0082ee00`) — documented as a normal
>   one-byte Channel-B frame for the original cmd; malformed RSP length/CRC is
>   rejected before any progress step advances.
