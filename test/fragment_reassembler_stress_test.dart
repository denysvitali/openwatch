...
void main() {
  group('ChannelB reassembler — stress', () {
    ...
    test('missing chunk: reassembly exceeds fragmentTimeout, state '
        'resets silently — NO NAK on Channel B (channel_b.dart:218-228)', () async {
      final t = _StubTransport();
      ...
      final f = Codec.buildChannelB(0x42, List<int>.generate(payloadLen, (i) => i));
    ...
    test('head-of-line blocking: parser is single-state, so an interleaved '
      ...
      final b = Codec.buildChannelB(0x04, List<int>.generate(30, (i) => 0x40 + i));
    ...
    test('duplicate chunk (replay storm): same frame pushed 5× emits '
        'EXACTLY once (FNV-1a LRU dedup, channel_b.dart:86-89 + 273-289)', () async {
      final t = _StubTransport();
      final p = ChannelBParser(t);
      p.bind();
      final emitted = <ChannelBCommand>[];
      ...
      final f = Codec.buildChannelB(0x27, [
        0xC2, 0x01, 0x01, 0x1E, 0x02, 0x5A, 0x03, 0x3C,
        0x04, 0x0F, 0x05, 0x29, 0x06,
      ]);
      for (var i = 0; i < 5; i++) {
        t.inB.add(f);
      }
    ...
    test('partial last chunk with bad CRC is SILENTLY dropped — no emit, '
      ...
      expect(t.sentA, isEmpty, reason: 'firmware never NAKs unsolicited frames');
      await sub.cancel();
      expect(emitted, isEmpty);
    });

    test('tiny payload (sub-MTU single chunk) emits in one frame; '
        'continuation state is never entered (channel_b.dart:210-211)', () async {
    ...
    test('reassembly timeout race: a chunk arriving 75% through the '
        'timer window CANCELS the discard (channel_b.dart:138 + '
        'channel_b.dart:218-228)', () async {
