...
void main() {
  ...
  group('DeviceCapabilities.fromSetTime', () {
    ...
    test('screen size encoded as little-endian u16', () {
      final caps = DeviceCapabilities.fromSetTime(setTimePayload({
        4: 0x40, // 64
        5: 0x01, // +256 = 320
        6: 0x40,
        7: 0x01,
  ...
  group('DeviceCapabilities.mergeSupport', () {
    ...
    test('alarm + dnd on pl[6]', () {
      ...
      final merged = DeviceCapabilities()
          .mergeSupport(supportPayload({6: 0xC0}));
    ...
    test('ultraviolet + realTimeHr on pl[7]', () {
      ...
      final merged = DeviceCapabilities()
          .mergeSupport(supportPayload({7: 0x09}));
    ...
    test('reduceFat + hideMessageNotification on pl[8]', () {
      ...
      final merged = DeviceCapabilities()
          .mergeSupport(supportPayload({8: 0x18}));
    ...
    test('wechatPay + menuWallpaper on pl[4]', () {
      ...
      final merged = DeviceCapabilities()
          .mergeSupport(supportPayload({4: 0x05}));
    ...
    test('aiAnalyze on pl[3] b7', () {
      final merged = DeviceCapabilities()
          .mergeSupport(supportPayload({3: 0x80}));
      expect(merged.aiAnalyze, isTrue);
    });

    test('temperatureTwoHundred on pl[0xa] b1', () {
      final merged = DeviceCapabilities()
          .mergeSupport(supportPayload({0x0a: 0x02}));
    ...
    test('takePhoto is inverted on pl[6] b2', () {
      expect(
        DeviceCapabilities()
            .mergeSupport(supportPayload({6: 0x00}))
            .takePhoto,
        isTrue,
      );
      expect(
        DeviceCapabilities()
            .mergeSupport(supportPayload({6: 0x04}))
            .takePhoto,
        isFalse,
      );
    });
    ...
    test('muslim flag honors either pl[1] b1 or pl[5] b7', () {
      expect(
        DeviceCapabilities()
            .mergeSupport(supportPayload({1: 0x02}))
            .muslim,
        isTrue,
      );
      expect(
        DeviceCapabilities()
            .mergeSupport(supportPayload({5: 0x80}))
            .muslim,
        isTrue,
      );
      expect(
        DeviceCapabilities()
            .mergeSupport(supportPayload({}))
            .muslim,
        isFalse,
      );
    });
